...
export default class Event {
    ...
    setServer(eventServer = false, healthcheck = "healthcheck") {
        if (!eventServer) return (this.earthWeb.eventServer = false);

        if (utils.isString(eventServer))
            eventServer = new providers.HttpProvider(eventServer);

        if (!this.earthWeb.isValidProvider(eventServer))
            throw new Error("Invalid event server provided");

        this.earthWeb.eventServer = eventServer;
        this.earthWeb.eventServer.isConnected = () =>
            this.earthWeb.eventServer
                .request(healthcheck)
                .then(() => true)
                .catch(() => false);
    }
    ...
    getEventsByContractAddress(
        ...
        if (contractAddress)
            routeParams.push(this.earthWeb.address.fromHex(contractAddress));
        ...
        return this.earthWeb.eventServer
            .request(

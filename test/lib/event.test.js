...
describe("EarthWeb.lib.event", async function() {
    let accounts;
    let earthWeb;
    let contractAddress;
    let contract;
    ...
    describe("#constructor()", function() {
        it("should have been set a full instance in earthWeb", function() {
            assert.instanceOf(earthWeb.event, EarthWeb.Event);
        });
    });
    ...
    describe("#getEventsByTransactionID", async function() {
        it("should emit an unconfirmed event and get it", async function() {
            ...
            let events;
            while (true) {
                events = await earthWeb.event.getEventsByTransactionID(txId);
                if (events.length) {
                    break;
                }
                await wait(0.5);
    ...
    describe("#getEventsByContractAddress", async function() {
        it("should emit an event and wait for it", async function() {
            ...
            let events;
            while (true) {
                events = await earthWeb.event.getEventsByContractAddress(
                    contractAddress,
                    {
                        eventName: "SomeEvent",

...
describe("EarthWeb.utils.base58", function() {
    describe("#encode58()", function() {
        it("should encode a buffer in base58 string", async function() {
            ...
            assert.equal(earthWeb.utils.base58.encode58(input), expected);
        ...
        it("should return '' or '1' if passing something different from a buffer", async function() {
            ...
            assert.equal(earthWeb.utils.base58.encode58([]), "");
            assert.equal(earthWeb.utils.base58.encode58("some string"), "");
            assert.equal(earthWeb.utils.base58.encode58({ key: 1 }), "1");
        });
    });
    ...
    describe("#decode58()", function() {
        it("should decode a base58 string in a buffer", async function() {
            ...
            const decoded = earthWeb.utils.base58.decode58(input);

            assert.equal(
                Buffer.compare(expected, Buffer.from(decoded, "utf-8")),
                0
            );
        });
        ...
        it("should return [] or [0] if passing something '' or '1'", async function() {
            ...
            assert.equal(
                JSON.stringify(earthWeb.utils.base58.decode58("")),
                "[]"
            );
            assert.equal(
                JSON.stringify(earthWeb.utils.base58.decode58("1")),
                "[0]"
            );
        });
    });
